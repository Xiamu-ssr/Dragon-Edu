...
public class MqMessageServiceImpl extends ServiceImpl<MqMessageMapper, MqMessage> implements MqMessageService {
    ...
    public List<MqMessage> getMessageList(int shardIndex, int shardTotal, String messageType,int count) {
        List<MqMessage> mqMessages = mqMessageMapper.selectListByShardIndex(shardTotal, shardIndex, messageType, count);
        List<Long> ids = mqMessages.stream().map(MqMessage::getId).toList();
        if (!ids.isEmpty()){
            mqMessageMapper.update(new LambdaUpdateWrapper<MqMessage>()
                .in(MqMessage::getId, ids)
                .setSql("execute_num = execute_num + 1")
            );
        }
        return mqMessages;
    }
